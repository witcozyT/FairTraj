...
class QuadNode(object):
    ...
    def insert(self, point):
        ...
        if not self.contains_point(point):
            warnings.warn(
                "Point {} can not fit in this node ({} - {}).".format(
    ...
    def find(self, point):
        ...
        if found_node is not None:
            ...
            #         return pnt, found_node
            return point, found_node
    ...
    def get_nodes(self):
        """
        返回以当前节点为根节点的子树下的叶子节点
        """
        if self.ll is None and self.lr is None and self.ul is None and self.ur is None:
            return [self]
        nodes = []
        nodes.extend(self.ll.get_nodes())
        nodes.extend(self.lr.get_nodes())
        nodes.extend(self.ul.get_nodes())
        nodes.extend(self.ur.get_nodes())
        return nodes
    ...
    def update_density(self, data):
        ...
        if self.ul is not None:
            if self.is_ul(node.center):
                self.ul.update_density(data)
            elif self.is_ur(node.center):
                self.ur.update_density(data)
            elif self.is_ll(node.center):
                self.ll.update_density(data)
            elif self.is_lr(node.center):
                self.lr.update_density(data)
            return
...
class QuadTree(object):
    ...
    def __init__(self, zone_range, capacity=None):
        """
        ...
        """
        lat_min, lon_min, lat_max, lon_max = [x * 1e6 for x in zone_range]
        ...
        self.width = width
        self.height = height
        self.center = self.convert_to_point((center_lat, center_lon))
    ...
    def get_nodes(self):
        ...
        返回四叉树中所有叶子节点的坐标
        """
        return self._root.get_nodes()
    
    def update_density(self, data):
        """
        更新四叉树中所有叶子节点的密度值
        """
        return self._root.update_density(data)
